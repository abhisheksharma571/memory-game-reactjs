...
const MemoryGame = () => {
  ...
  const [maxMoves, setMaxMoves] = useState(gridSize * 2);
  ...
  const initializeGame = () => {
    ...
    setSolved([]);
    setWon(false);
    setMoves(0); // Reset moves when the game restarts
  };
  ...
  const handleClick = (id) => {
    ...
    if (flipped.length === 0) {
      setFlipped([id]);
      setMoves(moves + 1); // Increment moves on first flip
      return;
    }
    ...
    if (flipped.length === 1) {
      ...
      if (id !== flipped[0]) {
        ...
        setMoves(moves + 1); // Increment moves on second flip
        checkMatch(id);
      } else {
        setFlipped([]);
        setDisabled(false);
    ...
    <div className="flex flex-col items-center justify-center min-h-screen bg-grey-100 p-4">
      ...
      <div className="mb-4 flex">
        ...
        <div className="">
          <label htmlFor="maxMoves" className="mr-2">
            Max Moves: (you can increases maxMoves upto 100 for easy winning) 
          </label>
          <input
            type="number"
            id="maxMoves"
      ...
      {won ? (
        ...
        <div className="mt-4 text-4xl font-bold text-red-600">
          Game Over! Max moves reached.
        </div>
      ) : (
        <div className="mt-2 text-xl font-semibold">Moves: {moves} / {maxMoves}</div>
      )}
